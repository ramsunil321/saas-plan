...
// See workspace-service/src/middlewares/error.middleware.ts for full docs.
// No multer in this service — notification endpoints do not accept file uploads.
...
export const errorMiddleware = (
  ...
  if (isAppError(error)) {
    if (error.isOperational) {
      logger.warn('[NotificationError] Operational error', { ...logContext, message: error.message });
    } else {
      logger.error('[NotificationError] Non-operational', { ...logContext, stack: error.stack });
    }
  ...
  logger.error('[NotificationError] Unhandled', { ...logContext, error: error.message, stack: error.stack });
  res.status(500).json({
    success: false,
    error: {
      code: 'INTERNAL_ERROR',
